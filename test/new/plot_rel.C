void plot_rel()
{
  ...
  filename.push_back("cv.dat");  
  filename.push_back("sd.dat");  
  filename.push_back("sk.dat");  
  filename.push_back("ku.dat");  
  filename.push_back("ko.dat");
  ...
  for (int i = 0; i < (int) filename.size(); i++)
    {
      stringstream file("");
      file << "set_r_rel/" << filename[i];

      //stringstream file2("");
      //file2 << "set_c_rel/" << filename[i];
  ...
  vector<string> label;
  label.push_back("Relative difference: Central Value");
  label.push_back("Relative difference: Standard Deviation");
  label.push_back("Relative difference: Skewness");
  label.push_back("Relative difference: Kurtosis");
  label.push_back("Relative difference: Kolmogorov");
