void plot2()
{
  ...
  filename.push_back("cv2_set_r.dat");
  ...
  for (int i = 0; i < (int) filename.size(); i++)
    {
      ...
      stringstream file4("");
      file4 << "set_c4/" << filename[i];
      stringstream file5("");
      file5 << "set_c5/" << filename[i];
      ...
      //TGraphErrors *g5 = new TGraphErrors(file5.str().c_str(),"%lg %lg %lg"); 
  ...
  vector<string> label;
  label.push_back("Total Central Value in #sigma units");
  ...
  for (int i = 0; i < (int) filename.size(); i++)
    {
      ...
      g->SetLineStyle(2);
      g->Draw("same,L");
      if (i == 0) l->AddEntry(g,"1#sigma interval","l");

      TGraph *gd = new TGraph(2);
      gd->SetPoint(0, 10, -1);
      gd->SetPoint(1, 1000, -1);
